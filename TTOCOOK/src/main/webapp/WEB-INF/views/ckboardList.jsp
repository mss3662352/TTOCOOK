        ...
        <div class="ck_board">
            ...
            <table class="ck_boardTbl">  
                ...
                <c:forEach var="boardDto" items="${list}">
			       	<tr class="ck_content">
			           <td>${boardDto.ckbno}</td>
			           <td>${boardDto.ckbwriter}</td>
			           <td><a href="<c:url value='/board/ckread${ph.sc.getQueryString() }&ckbno=${boardDto.ckbno } '/>">
			           		<c:out value="${boardDto.ckbtitle}"/>
			           </a></td>
			           <td>X</td>
			           <c:choose>
				            <c:when test="${boardDto.ckbdate.time >= startOfToday}">
				              	<td class="regdate"><fmt:formatDate value="${boardDto.ckbdate}" pattern="HH:mm" type="time"/></td>
				            </c:when>
				            <c:otherwise>
				              	<td class="regdate"><fmt:formatDate value="${boardDto.ckbdate}" pattern="yyyy-MM-dd" type="date"/></td>
				            </c:otherwise>
		          		</c:choose>
			           <td>${boardDto.ckview_cnt}</td>
			           <td>${boardDto.ckgood_cnt}</td>
			       </tr>
				</c:forEach>
            ...
            <button class="ck_write" onclick="location.href='<c:url value="/board/ckwrite"/>'">글 작성</button>
        </div>
